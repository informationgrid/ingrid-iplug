...
#exec "$JAVA" $JAVA_HEAP_MAX $INGRID_OPTS -classpath "$CLASSPATH" $CLASS --multicastPort 11111 --unicastPort 11112
exec "$JAVA" $JAVA_HEAP_MAX $INGRID_OPTS -classpath "$CLASSPATH":./ $CLASS --descriptor $INGRID_HOME/conf/jxta.conf.xml --busurl 
